...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)
        x = self.drop(x)
        # print("after drop x: ", x.shape)
        # Reshape back to [batch, channels, h, w, d] 
        # x = x.view(batch_size, channels, *x.shape[1:])
        return x
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # print("====self.window_size:",self.window_size[2])
...
class AttenBlocks(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attn = TransformerCrossAttentionBlock(dim, struc_dim, num_heads)
    ...
    def forward(self, x, x_size, params, struc_feat=None):
        ...
        """
        # print("before blk x_size: {}".format(x_size))
        for blk in self.blocks:
            x = blk(x, x_size, params['rpi_sa'], params['attn_mask'])

        # Apply global attention block (GAB)
        y = self.gab(x, x_size, params['rpi_sa'], params['attn_mask'])
        x1 = x + y * self.scale  # Scale the output and add residual
        # add cross_attn
        x = self.cross_attn(x, struc_feat)
...
class RHTB3D(nn.Module):
    ...
    def forward(self, x, x_size, params, struc_feat=None):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params, struc_feat), x_size))) + x
...
class HMANet3D(nn.Module):
    ...
    def set_loss(self):
        self.loss_func = None
    def forward_features(self, x, struc_feat=None):
        ...
        for layer in self.layers:
            x = layer(x, x_size, params, struc_feat)
    ...
    def forward(self, x,istrain=True,struc_feat=None):
        ...
        elif istrain and struc_feat ==None:
            label = x['label']
            density = x["density"]
        else:
            density = x
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            if istrain:
                x = self.conv_first(density)
            else:
                x = self.conv_first(x)
            x = self.conv_after_body(self.forward_features(x, struc_feat)) + x
